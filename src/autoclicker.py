...
class AutoClicker:
    ...
    def drop_items(self):
        if not self.config.auto_drop_settings['enabled']: return
        pyautogui.press("f")
        time.sleep(0.2)
        ...
        for item in items:
            search_icon_location = None
            try:
                search_icon_location = pyautogui.locateOnScreen('../assets/search_icon.png', confidence=0.9, region=region)
            except:
                return
            ...
            except pyautogui.ImageNotFoundException: return
